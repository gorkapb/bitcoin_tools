...
def parse_ldb(fout_name, fin_name=CFG.chainstate_path, decode=True):
    ...
    o_key = db.get(bytes(unhexlify("0e00").decode("utf-8") + "obfuscate_key", 'UTF-8'))
...
def aggregate_dust_np(fin_name, fout_name="dust.json", fltr=None):
    ...
    for line in fin:
        ...
        if not fltr or (fltr and fltr(data)):
            ...
            if data['non_profitable_est'] is not None:
...
def check_native_taproot(script):
    """
    Checks whether a given output script is a native taproot type.

    :param script: The script to be checked.
    :type script: str
    :return: tuple, (True, taproot type) if the script is a native taproot, (False, None) otherwise
    :rtype: tuple, first element boolean
    """

    if len(script) == 34*2 and script[:4] == "5120": #OP_PUSHNUM_1 OP_PUSHBYTES_32
        return True, "P2TR"
...
def get_min_input_size(out, height, count_p2sh=False, coin="bitcoin", compressed_pk_height=0):
    ...
    if out_type == 0:
        ...
        taproot = check_native_taproot(script)
        # P2MS
        if check_multisig(script):
            ...
            scriptSig = 27 # PUSH sig (1 byte) + sig (71 bytes) + PUSH pk (1 byte) + pk (33 bytes) (106 / 4 = 27)
            scriptSig_len = 1
        elif taproot[0] and taproot[1] == "P2TR":
            scriptSig = 57.5 # PUSH sig (1 byte) + sig (71 bytes) + PUSH pk (1 byte) + pk (33 bytes) (106 / 4 = 27)
            scriptSig_len = 1
        else:
